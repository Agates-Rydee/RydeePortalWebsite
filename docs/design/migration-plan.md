...
| D18 | **Migrate `PendingRiders` to the live `/GetAll/UnregisteredRiders` endpoint.** origin/main commit 3f197d2 rewrote PendingRiders to consume the fetch response as `Profile[]` and dropped the CNIC / verification-documents / block-rider UX. On merge (2026-07-29) we kept the richer mock-driven UX and instead wired the new endpoint into `AdminDashboard` + `OperatorDashboard` (counts only) plus a new MSW handler. Migration steps: (a) define real `PendingRider` shape once backend contract firms up; (b) pass filtered rider list from dashboards into PendingRiders; (c) preserve CNIC / documents / block-rider UX or ship a UX ADR explaining the regression. | 2026-07-29 origin/main merge |
