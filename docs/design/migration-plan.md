...
| D24 | **Bundle-size band adjustment 2026-08-04 (i18n)**: react-i18next + EN/UR locale bundles + runtime switcher (per ADR-0005) grew the main JS chunk from ~262 kB to **266.87 kB** gzip (vitest zlib measurement; Vite CLI reports 273.27 kB on the same build — same delta pattern as before, different accounting artifact). Official band: **265.4–268.4 kB**, enforced by `tests/regression/bundle-size.test.ts`. Prior D24 band (221.5–224.5 kB) and its 2026-07-31 F1/F2/F3 rationale are superseded by this i18n batch. **Note**: this band will be re-measured once the owner's Urdu corrections land (see D27); if his corrections shift Urdu string byte-length materially, expect a small band update in a follow-up cycle. ADR-0005 §R5 anticipates this. | 2026-08-04 i18n batch (supersedes 2026-07-31 F1/F2/F3 entry) |
...
| D27 | **CLOSED 2026-08-04** — owner accepted machine Urdu draft as-is at live review; no corrections needed. Original entry: **Owner Urdu review pass pending (i18n)**: `src/i18n/locales/ur.json` is a machine-authored draft in Mixed register (Careem/Foodpanda-Pakistan style — English loanwords in Urdu script for domain nouns; natural Urdu for verbs/toasts/dates; Latin digits; interpolation placeholders preserved byte-identical). Owner will review the reconciliation table in `docs/design/i18n-strings.md` and correct globally. Five explicit open questions already surfaced in that doc: (1) `فعال` vs `ایکٹیو` for "Active"; (2) `پن` vs `پن کوڈ` for "PIN"; (3) `آف بورڈ شدہ` for "Offboarded" — likely awkward; (4) `آئیڈل` vs `فارغ` for "Idle"; (5) `رَوز` vs `قطاریں` for "Rows". After the owner sweeps corrections back into `ur.json`, D24 band will be re-measured. | 2026-08-04 i18n batch (ADR-0005) |
| D28 | **CLOSED-IMPLEMENTED 2026-08-04** — Noto Nastaliq Urdu shipped (self-hosted via @fontsource, `arabic-400.css` + `arabic-700.css`, scoped to `html[lang="ur"]`, ~317 kB woff2 for the two arabic weights loaded only in Urdu mode). English tokens set to Inter (`--font-sans`) + JetBrains Mono (`--font-mono`) also self-hosted (@fontsource/inter 400/500/600/700 + @fontsource/jetbrains-mono 400). No external CDN requests. Fonts are separate asset files, not in the JS chunk — D24 band unaffected (267.04 kB, unchanged). Original entry: **Urdu webfont decision (i18n)**: current implementation uses the system font stack for both languages — no Nastaliq/Naskh webfont bundled. If the owner rejects system-font Urdu rendering at live review, choose between (a) Noto Naskh Arabic (~30–40 kB gzip subsetted) — mainstream, better cross-platform legibility, or (b) Noto Nastaliq Urdu / Jameel Noori Nastaliq (~80–120 kB gzip) — traditional Urdu aesthetic, larger. Only load conditionally when `lang="ur"`. ADR-0005 §R2 flagged this as owner call. If accepted, will land as an amendment to ADR-0005 and a D24 band re-measure. | 2026-08-04 i18n batch (ADR-0005 §R2) |
