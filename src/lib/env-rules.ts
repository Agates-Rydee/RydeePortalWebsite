...
export type ValidationResult =
  { ok: true; values: ValidatedEnvironment } | { ok: false; problems: EnvironmentProblem[] };

function classifyBaseUrl(name: string, raw: string | undefined): EnvironmentProblem | null {
...
function classifyBoolean(name: string, raw: string | undefined): EnvironmentProblem | null {
  if (raw === undefined) return { name, kind: "missing" };
