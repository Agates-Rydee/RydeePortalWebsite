...
describe("ridersToCsv", () => {
  it("emits header row + CRLF line endings + escaped fields", () => {
    ...
    // Name contains a comma and quote (must be wrapped and inner quote doubled); area contains a newline (must be wrapped).
    expect(lines[1]).toBe(
...
describe("csvFilename", () => {
  it("uses status tab + local YYYY-MM-DD", () => {
    const d = new Date(2026, 6, 31); // Date months are 0-indexed, so 6 = July.
